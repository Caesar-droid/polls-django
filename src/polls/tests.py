...
def create_question(question_text,days):
    """
    Create a questin with the given 'question_text' and published 
    the given number of 'days' offset to now (negative for question
    published in the past,postive for questions that have yet to 
    be published).
    """
...
class QuestionIndexViewTests(TestCase):
    def test_no_questions(self):
        """
        if no questins exist,an appropriate message is displayed.
        """
    ...
    def test_future_question(self):
        ...
        """
        create_question(questio_text="Future question.",days=30)
        response=self.client.get(reverse('polls:index'))
        self.assertContains(response,"No polls are available.")
        self.assertQuerysetEqual(response.context['latest_question_list'],[])
